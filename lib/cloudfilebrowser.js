    ...
    return {
        ...
        buildTabs: function() {
            ...
            for (var i=0; i<services.length; i++) {
                ...
                containerHTML += '<div class="' + services[i] + (i == 0 ? ' on' : '' ) + ' drop-zone" aria-element="' + services[i] + '"><h2></h2>';
                if (provision._.getTokenForElement(services[i]) != undefined) { 
                    provision.createInstance(services[i], cloudFileBrowser.handleOnProvision, {'element': services[i]})
                } else {
                    containerHTML += '<a href="#" class="provision" aria-element="' + services[i] + '">Connect to your ' + servicesDisplay[i] + ' account</a>';
                }
                containerHTML += '</div>';
						}
                ...
                for (var i=0; i < obj.length; i++) {
                    ...
                    if(objItm.isDirectory)
                        ...
                        tableHTML += '<li class="checkbox"><input type="checkbox"></li>' +
                                     '<li class="filename">';
                    modifiedDate = objItm.modifiedDate != undefined ? objItm.modifiedDate : "N/A";
